...
| Client | `ghl_location_id` | tenant_id | Channels |
| --- | --- | --- | --- |
| The Bot Crew | `wRMDr6h3anwYpM64XAUe` | `04385692-5c0d-436e-af77-4b1aa3fcc223` | whatsapp, instagram, facebook — **live, no entry gate** (`trigger_keywords` NULL since 2026-08-20: the Sprint de Bótox ad is click-to-WhatsApp, a keyword would drop leads). No campaign variants (`keyword_variants`/`prompt_variants` NULL), `demo_sessions_enabled=false`. `demo_on_keywords={demo botox}` flips the Alenza Med Spa roleplay for ANY sender. `houseRules` carries the fit filter, the call-offer rule and the absolute rules; `offering` carries the Sprint (price, guarantee, objections); `qualificationNotes` the flow. Calendar `Llamada con Leo` (20-min slot, `booking_horizon_days=3`, `lead_timezone_enabled=true` — leads nationwide, calendar in Tijuana; see business-logic §5d). The whole `prompt_overrides` is mirrored in `evals/fixtures.ts` (`BOT_CREW_PERSONA`) and drift-checked. See [`config-model.md`](config-model.md) § Where to put a given piece of text. |

...
| Client | `ghl_location_id` | tenant_id | Channels |
| --- | --- | --- | --- |
| The Bot Crew | `wRMDr6h3anwYpM64XAUe` | `04385692-5c0d-436e-af77-4b1aa3fcc223` | whatsapp, instagram, facebook — **live, no entry gate** (`trigger_keywords` NULL since 2026-08-20: the ad is click-to-WhatsApp, a keyword would drop leads). No campaign variants (`keyword_variants`/`prompt_variants` NULL), `demo_sessions_enabled=false`. **Offer changed 2026-09-07:** the Sprint de Bótox (ads + lead gen + 10-appointment guarantee, $3,000/mo) was replaced by the response system — answering WhatsApp/FB/IG 24/7, booking when the business takes appointments, follow-up — at $500/mo that covers only the tools (Leo's work is free; 5 spots, frozen price). No ads, no lead gen, and **nobody is disqualified any more**: the flow captures giro / message volume / who answers today / main channel and books the call. The botox demo was retired with it (`demo_on_keywords`, `demo_off_keywords`, `demo_prompt_overrides` all NULL). `houseRules` carries who-it-serves, the call-offer rule and the absolute rules; `offering` carries the offer (price, what it is NOT, objections); `qualificationNotes` the flow. Calendar `Llamada con Leo` (20-min slot, `booking_horizon_days=3`, `lead_timezone_enabled=true` — leads nationwide, calendar in Tijuana; see business-logic §5d). The whole `prompt_overrides` is mirrored in `evals/fixtures.ts` (`BOT_CREW_PERSONA`, regenerated with `node workers/scripts/sync-botcrew-fixture.mjs`) and drift-checked. See [`config-model.md`](config-model.md) § Where to put a given piece of text. |
...
| Dr. Heriberto Valdivia | `rfL7uM3c5mpfIUGxCR3C` | `bfad47bf-ccec-4820-a3ed-1f9dc964f3f1` | **silent** (seeded 2026-08-28, `enabled_channels` NULL). Medicina estética y regenerativa, Chihuahua, `America/Chihuahua`. Bot books: `calendars` keys `Consulta` (everything aesthetic — the first consult includes the valoración) and `Consulta de Bariatría`, both → the one GHL calendar `qh8yxxSnb4DAo72obVmG` (30-min slots, matches `hours`, offset -06:00 verified 2026-08-28). Own key `ai_key_ref='HERIBERTO'`. Persona "Sofía"; `houseRules` = medical limit (GLP-1: never a drug name/dose) + nothing invented. Mirrored in `evals/fixtures.ts` (`HERIBERTO_PERSONA`). Site: `sites/dr-valdivia/`. |

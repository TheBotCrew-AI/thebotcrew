...
- [x] **Testing harness & deploy gate (Layers 1–2 + CI)** — `pnpm test:unit` (no keys/network/DB): Layer 1 = webhook parsers + signature verify (`verifyDetached` keypair round-trip) + gate helpers/keyword matcher (`ghl/webhook.test.ts`, `core/tenant.test.ts`); Layer 2 = mocked-seam orchestration of `handleInboundWebhook` — dedup, suppression, channel/keyword gates, happy path, delivery failure (`worker/webhook-handler.test.ts`). 35 tests. CI (`.github/workflows/ci.yml`) runs typecheck + `test:unit` on every push/PR.
  - [ ] **Layer 3 — golden-conversation regression set** (live, API-key gated): multi-turn scripted conversations with deterministic + LLM-judge assertions (qualification, booking-calls-getAvailability, anti-hallucination, opt-out, handoff). Becomes the staging deploy gate. Needs staging env.
  - [ ] Layer 1 for the FAQ matcher + prompt rendering + status classifier gate (not yet covered).
...
- [x] **Durable turn processing (reconciliation net)** — `worker/reconciliation.ts` + `app_load_unanswered_turns` (migration 0020): a per-minute cron sweep finds conversations whose latest message is an unanswered inbound (>45s old, active, gates re-applied) and re-runs `runAgentTurn`. Atomic claim (`reconcile_claimed_at`, FOR UPDATE SKIP LOCKED + 120s cooldown) + latest-message gate ⇒ no double-send. Validated: an induced dropped turn was recovered in ~1.5 min, single reply.

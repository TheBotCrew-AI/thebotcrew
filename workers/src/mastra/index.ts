...
export const mastra = new Mastra({
  ...
  server: {
    apiRoutes: [
      ...
      // The latest info-gap report for a tenant. Read-only. The key is PER TENANT and
      // lives in tenant_config.report_key (0055): generated by the DB, no Worker secret to
      ...
      registerApiRoute('/reports/info-gaps/:tenantId', {
        ...
        handler: async (c) => {
          ...
          const expected = await loadTenantReportKey(tenantId);
          const auth = c.req.header('authorization') ?? '';
